...
def train_modelYin(ranker, train_dat_path):
    print("ranker:", ranker)
    print("Training file path: {0}, whether it exists: {1}".format(train_dat_path, os.path.exists(train_dat_path)))
    dat_file = str(os.path.split(train_dat_path)[-1])
    save_model_path = os.path.join(path, "model")
    model_name = str(dat_file.split("+")[0]) + ".dat"
    ...
    try:
        ...
        print("The training command line execution result is：" + str(exc.returncode))
    finally:
        return model_path
        pass
    pass
...
def pred_test_dataYin(test_dat_path, model_path):
    # 用上面建立好的模型来预测我们的测试数据
    print("test_dat_path path: {0}, and whether it exists: {1}".format(test_dat_path, os.path.exists(test_dat_path)))
    print("Model path: {0}, and whether it exists：{1}".format(model_path, os.path.exists(model_path)))
    ...
    result_dat_name = "result_" + str(test_dat_name.split("+")[0]) + ".dat"
...
def get_pred_bug(result_dat_path):
    print("result_dat_path path: {0}, and whether it exists: {1}".format(result_dat_path, os.path.exists(result_dat_path)))
    pred_bug = []
    #lines = open(result_dat_path, "r", encoding="utf-8").readlines()
